 ...
 * @author Sachintha
 */
public class Map_Proj_Vehicle extends javax.swing.JFrame {

    private final MysqlConnect _dbConnection;
    private final SimpleDateFormat dateformat;

    public Map_Proj_Vehicle() {
...
//        GetModel();
    ...
    private void GetSupplier() {
        try {
            ResultSet rs = _dbConnection.query("SELECT * FROM reg_vehicle  WHERE st='A' and vid not in "
                    + "(select vid from map_proj_vehicle where pid='" + cmbProject.getSelectedItem().toString().split("--")[0] + "' and st <> 'D')");
            cmbVehical.removeAllItems();
            while (rs.next()) {
                cmbVehical.addItem(rs.getString("vid") + "--" + rs.getString("name"));
            }
        } catch (SQLException ex) {
            Logger.getLogger(Reg_Employee.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jButton2.setText("Project-Vehicle");
        ...
        jPanel1.add(cmbVehical);
        cmbVehical.setBounds(110, 70, 240, 20);
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        try {
            if (jTable1.getSelectedRow() < 0) {
                JOptionPane.showMessageDialog(rootPane, "Please select Vehical from grid");
                return;
            }
            String part1 = jTable1.getValueAt(jTable1.getSelectedRow(), 0).toString();
            System.out.println(part1);
            String sql = "delete from map_proj_vehicle WHERE mid =" + part1 + "";
...
//            GetModel();
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        try {
            ...
            if (!cmbVehical.getSelectedItem().toString().contains("--")) {
                throw new Exception("Empty VehicalID");
            }

            String sql = "INSERT INTO `map_proj_vehicle` ("
                    + "`mid`," //1
                    + "`pid`," //1
                    + "`vid`,"//2
                    + "`st`"//3
                    ...
                    + "'" + cmbVehical.getSelectedItem().toString().split("--")[0] + "',"
                    + "'A')ON DUPLICATE KEY UPDATE "
                    + " pid='" + cmbProject.getSelectedItem().toString().split("--")[0] + "',"
                    + " vid='" + cmbVehical.getSelectedItem().toString().split("--")[0] + "',st='A'";
            int tmp = _dbConnection.insert(sql);
            ...
            Logger.getLogger(Map_Proj_Vehicle.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void loadTable(String part1) {
        try {
            String sql = "select * from ("
                    + "SELECT a.mid ID,a.`pid` ProjectID,b.`name`AS ProjectName,c.`name` VehicalName,c.`address` VehicalAddress,c.`licence` LicenceNo,\n"
                    + "c.`dname` DriverName,c.`dlnum` DriverLicence,c.`vehicle_type` VehicalType\n"
                    + "FROM `map_proj_vehicle` a\n"
                    + "INNER JOIN `reg_projects` b ON a.`pid`=b.`proid` AND b.`st` != 'D'\n"
                    + "INNER JOIN `reg_vehicle` c ON a.vid=c.`vid` AND c.`st` != 'D'"
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Map_Proj_Vehicle.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Map_Proj_Vehicle().setVisible(true);
            }
        });
    }
